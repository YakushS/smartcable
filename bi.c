...
//------------------------------------------------------------------------------
#define RESET_CRC_SUM( CRCSUM ) CRCSUM = 0xFFFFFFFF;
#define ADD_TO_CRC_SUM( CRCSUM, x )\
CRCSUM =((CRCSUM)>>8) ^ CRCCodeTabl[ (unsigned char)(CRCSUM)^(unsigned char)(x) ];
...
const unsigned long CRCCodeTabl[256]={
...
//------------------------------------------------------------------------------
void bi_write_byte(uint8_t byte)
{
    BI_UART->DR = byte;
    while( !(BI_UART->SR & UART_FLAG_TC) );
}
//------------------------------------------------------------------------------
uint8_t bi_read_byte(void)
{
    while(!(BI_UART->SR & UART_FLAG_RXNE));
    return BI_UART->DR;    
}
//------------------------------------------------------------------------------
bool bi_read_cmd(unsigned char *p_Buf)
{
    unsigned short cntr, tmp, timeout;
    unsigned char *p_bt_buf;

    tmp = BI_UART->DR;

    for(timeout=RX_BYTE_TIMEOUT; timeout; timeout--) // ~550 циклов до приёма (115200 при 120MHz без оптимизации)
    {
        if(BI_UART->SR & UART_FLAG_RXNE) {break;}
        if(timeout==1) {return false;}
    }
    //while(!(BI_UART->SR & USART_FLAG_RXNE));
    tmp |= ((unsigned short)BI_UART->DR<<8);
    ...
    while(cntr--)
	{
		for(timeout=RX_BYTE_TIMEOUT; timeout; timeout--)
        {
            if(BI_UART->SR & UART_FLAG_RXNE) {break;}
            if(timeout==1) {return false;}
        }
    ...
    for( cntr = 0; cntr < 4; cntr++ )
	{
        ...
        //while(!(BI_UART->SR & USART_FLAG_RXNE));
        CRCsumRCV.b[cntr] = BI_UART->DR;
	}

    RESET_CRC_SUM( CRCsumCALC.l );
    ADD_TO_CRC_SUM( CRCsumCALC.l, (unsigned char)tmp );
    ADD_TO_CRC_SUM( CRCsumCALC.l, (unsigned char)(tmp>>8));

    p_bt_buf = p_bt_buf - tmp;
    while(tmp--) {ADD_TO_CRC_SUM( CRCsumCALC.l, *(p_bt_buf++) );}
	for( cntr = 0; cntr < 4; cntr++ )
	{
        BI_UART->DR = CRCsumCALC.b[cntr];
        while( !(BI_UART->SR & UART_FLAG_TC) );
	}

    if(CRCsumCALC.l == CRCsumRCV.l)
    {
        return true;
    }
...
void bi_write(unsigned char *buf, unsigned short n)
{
    ...
    while( !(BI_UART->SR & UART_FLAG_TXE) );

	// передать размер
	BI_UART->DR = n; ADD_TO_CRC_SUM( CRCsumCALC.l, n ); while( !(BI_UART->SR & UART_FLAG_TC) );
	BI_UART->DR = n>>8; ADD_TO_CRC_SUM( CRCsumCALC.l, n>>8 ); while( !(BI_UART->SR & UART_FLAG_TC) );
	// передать данные
	while( n-- )
	{
		BI_UART->DR = *buf;	
		ADD_TO_CRC_SUM( CRCsumCALC.l, *(buf++) );
		while( !(BI_UART->SR & UART_FLAG_TC) );
	}
	// передать CRC
	for( unsigned char i = 0; i < 4; i++ )
	{
		BI_UART->DR = CRCsumCALC.b[i]; while( !(BI_UART->SR & UART_FLAG_TC) );
	}
